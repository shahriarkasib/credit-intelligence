...
Table names match Google Sheets exactly for simplicity:

| Category | Tables |
|----------|--------|
| Core | `runs` |
| Workflow | `llm_calls`, `tool_calls`, `langgraph_events`, `plans`, `prompts`, `data_sources`, `assessments` |
| Evaluation | `evaluations`, `tool_selections`, `consistency_scores`, `cross_model_eval`, `llm_judge_results`, `agent_metrics`, `coalition`, `log_tests` |
| System | `api_keys` |
...
| Table | Description |
|-------|-------------|
| `llm_calls` | All LLM API calls with prompts, responses, tokens, costs |
| `tool_calls` | Tool executions with inputs, outputs, timing |
| `langgraph_events` | LangGraph workflow events and state changes |
| `plans` | Task plans generated by the planner |
| `prompts` | Prompts used with variables |
| `data_sources` | Data fetched from external sources |
| `assessments` | Credit assessments produced |

### Evaluation Tables

| Table | Description |
|-------|-------------|
| `evaluations` | General evaluation scores |
| `tool_selections` | Tool selection precision/recall/F1 |
| `consistency_scores` | Cross-run consistency metrics |
| `cross_model_eval` | Multi-model comparison results |
...
| `log_tests` | Log verification results |
...
curl "https://your-app.herokuapp.com/pg/query/llm_calls?run_id=abc-123-def&limit=50"

# Get all evaluations
curl "https://your-app.herokuapp.com/pg/query/evaluations?limit=100"
```
...
SELECT
  r.*,
  (SELECT json_agg(l.*) FROM llm_calls l WHERE l.run_id = r.run_id) as llm_calls,
  (SELECT json_agg(t.*) FROM tool_calls t WHERE t.run_id = r.run_id) as tool_calls,
  (SELECT json_agg(e.*) FROM langgraph_events e WHERE e.run_id = r.run_id) as events,
  (SELECT row_to_json(a.*) FROM assessments a WHERE a.run_id = r.run_id LIMIT 1) as assessment,
  (SELECT row_to_json(ev.*) FROM evaluations ev WHERE ev.run_id = r.run_id LIMIT 1) as evaluation
FROM runs r;
